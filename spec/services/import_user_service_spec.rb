...
RSpec.describe ImportUserService, :vcr do
  ...
  context 'when everything goes normally' do
    it 'creates the user, their posts, and comments' do
      expect { service.call }
        ...
        .and change(Post, :count).by_at_least(1)
        .and change(Comment, :count).by_at_least(1)

      user = User.find_by(username: username)

      expect(user).to be_present
      expect(user.posts.count).to be > 0
      expect(user.posts.first.comments.count).to be > 0
    end
  end
  ...
  context 'when the user already exists' do
    ...
    it 'does not create a duplicate user, but imports posts/comments' do
      expect {
        service.call
      }.to change(Post, :count).by_at_least(1)
      .and change(Comment, :count).by_at_least(1)

      expect(User.where(username: username).count).to eq(1)
      expect(user.reload.posts.count).to be > 0
    end
  end
  ...
  context 'when there is an error creating a post' do
    before do
      allow_any_instance_of(User).to receive_message_chain(:posts, :find_or_create_by!).and_raise(StandardError, 'Post error')
      allow(Rails.logger).to receive(:error)
    end
  ...
  context 'when there is an error creating a comment' do
    before do
      allow_any_instance_of(Post).to receive_message_chain(:comments, :find_or_create_by!).and_raise(StandardError, 'Comment error')
      allow(Rails.logger).to receive(:error)
    end
